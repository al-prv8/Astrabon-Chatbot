        ...
        {!isExpanded && (
          ...
          >
            ...
            {chatHistory.length > 0 && (
              <span className="absolute -top-1.5 -left-1 w-5 h-5 rounded-full bg-primary text-text-on-primary text-[10px] font-bold flex items-center justify-center">
                {Math.min(chatHistory.filter(m => m.sender === 'bot').length, 9)}
              </span>
            )}
          </motion.button>
        )}
